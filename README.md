...
## Automated Acceptance Testing (automated acceptance testing started on 11/06/2025)
- The ambassador was not built using acceptance test driven development. The application as a whole is not tested, the request making & filtering logic has been developed cohesively and isolated. The isolation of request-making and filtering logic combined with the "ports and adapters" pattern has allowed this to be bulletproof assuming the core use case classes are used in the driving adapter code (reverse proxy/http server). This logic has been tested to be correct in isolation, external to dependencies on third party code. This has allowed us a level of testing and correctness comparable to end-to-end acceptance testing, at the unit level, which is part of why the application was not built with acceptance tests first.
  - As a consequence of the ambassador not being built acceptance test first, the ones we are retrofitting onto it gradually can't be used alone to thoroughly falsify the releasability of the ambassador. 
- The ambassador is being retrofit with acceptance tests. These tests will comprehensively test each feature works on the application, examples will include, but not limited to:
  - Does the application forward game packets to the upstream server?
  - Does the application forward game packets from the upstream server to the client?
  - Does the application filter the SWF samples as expected for the various upstream base urls.
    - Does it filter them all for the main content cdn?
    - Does it filter them if they're in a response from the main api?
    - Does it filter them from all of them? Does it strip the transfer-encoding header when present upstream
- A misconception about clean architecture is that automated testing at a separation of concerns is enough, it is not. All of the important logic is isolated and independently testable, put together in a way that critical logic will not go wrong (in our case, making a request, plus filtering the response), the issue is, this correctness doesn't address the presentation and infrastructure logic of the separation of concerns, simple plumbing logic (like dependency injection) can break the application, or use cases may never be called, among many other issues.
    - Without acceptance tests we don't receive critical feedback about the application, we know that when a packet from the game server is handled, it will be sent to the client, but we don't know everything is wired up correctly to facilitate this. 
- The acceptance testing plan: We're going to add acceptance tests to the ambassador retroactively to ease further development and to get feedback on the driver adapter portions of the application.
## End Goal/Progress
